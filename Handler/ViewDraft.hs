...
getViewDraftR draftId = do
    ...
    let pickmap = Map.fromList $ map (\p -> (p ^. _1 . pickNumber, p)) picks
        ...
        lastRow =
            min (fromIntegral $ view draftRounds draft -1) . fst
                . pickNumToRC draft
                $ Map.size pickmap
        ...
        prettyPick pick details =
            let pclass
                    | Just (_pickDrafter pick) /= muid && isReserved (_cardCard details) = Just "bg-danger"
                    | otherwise = Nothing
...
getAllowedCards did cuid = map munge <$> runDB query
  where
    ...
    query = E.select $
        E.from $ \(cubeEntry `E.InnerJoin` card) -> do
