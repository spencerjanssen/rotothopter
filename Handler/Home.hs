...
userRankings userId = map munge <$> query
  where
    ...
    query = select $
        from $ \(ranking `InnerJoin` cube) -> do
...
featuredCubes = select $
    from $ \(featuredCube `InnerJoin` cube) -> do
